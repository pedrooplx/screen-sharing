...
9. **`out/main` é CommonJS (`index.cjs`), não ESM.** Descoberto no primeiro build empacotado (Fase 4, `npm run dist:win`): o processo `main` como ESM (o default sob `"type": "module"`) simplesmente **não executava** quando carregado de dentro de um `.asar` — nem a primeira linha do módulo rodava, sem nenhum erro em lugar nenhum (o app só mostrava uma janela de erro genérica do próprio Electron). O mesmo build, fora do asar, funcionava perfeitamente. `electron.vite.config.ts` força `format: 'cjs'` pro `main` (igual já era feito pro `preload`); `import.meta.dirname` (que não sobrevive à conversão pra CJS) virou `dirname(fileURLToPath(import.meta.url))`, que o Rollup sabe converter.
10. **`scripts/patch-binary-data.mjs` reescreve `node_modules/@shinyoshiaki/binary-data`** (dependência do `werift`) depois de todo `npm install`. O pacote vendoriza `lib/`/`types/`/`internal/` privadamente via um `node_modules` aninhado dentro do próprio `src/`, resolvido com `require('lib/algo')` — um truque válido num filesystem de verdade, mas que o coletor de dependências do electron-builder descarta silenciosamente (nada declara esse `node_modules` aninhado como dependência real em lugar nenhum), não importa a combinação de `files`/`asarUnpack`/`extraResources` tentada. Resultado sem o patch: `Cannot find module 'lib/binary-stream'` só no app empacotado. O patch renomeia a pasta pra fora do nome mágico `node_modules` e reescreve os `require()` pra caminhos relativos — resolve na raiz, funciona com qualquer empacotador.
